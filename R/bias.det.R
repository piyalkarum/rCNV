...
get.pvals<-function(x,df,p.cal){
  ...
  if(nrow(snp1het)>3){
    ...
    chi.het<-sum((((n*p.sum)-snp1het[,2])^2/n*p.sum)+(((n*(1-p.sum))-snp1het[,1])^2/n*(1-p.sum)),na.rm = TRUE)
    chi.het.sum<-chi.het
    chi.05<-sum((((n*p.05)-snp1het[,2])^2/n*p.05)+(((n*(1-p.05))-snp1het[,1])^2/n*(1-p.05)),na.rm = TRUE)
    chi.05.sum<-chi.05
    chi.all<-sum((((n*p.all)-snp1het[,2])^2/n*p.all)+(((n*(1-p.all))-snp1het[,1])^2/n*(1-p.all)),na.rm = TRUE)
    chi.all.sum<-chi.all
