...
dupsvd<-function(xx,v95,v05){
  val<-xx[4]
  coord<-as.numeric(unlist(xx[c(9,6)]))
  if(coord[1]>1000){
    coord[1]<-1000
  }
  if(coord[2]>400){
  ...
  if(val>v95[coord[1],coord[2]] | val<v05[coord[1],coord[2]]){
    dps<-"duplicated"
  } else {
    dps<-"singleton"
  }
...
#' @param dup.info data.frame of duplication info generated by dup.snp.info function
#' @param plot logical. whether to plot the duplicated and singleton snps
#'
#' @return a dataframe of snps with their status of duplication
#'
...
#' duplicated<-sig.snps(dup.info)
#'
#' @export
sig.snps<-function(dup.info,plot=TRUE){
  sts<-apply(dup.info,1,dupsvd,v95,v05)
  (d<-cbind(dup.info[,c(1:4,10:12)],dup.stat=sts))
  if(plot){
    cols<-makeTransparent(c("red","black"),alpha=0.5)
    d$Color <- cols[2]
