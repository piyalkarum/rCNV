...
dupsvd<-function(inx){
  ...
  if(val>v95[coords[1],coords[2]] | val<v05[coords[1],coords[2]]){
    dps<-"duplicated"
  } else {
    dps<-"singleton"
  }
...
sig.hets<-function(dup.info,plot=TRUE){
  ...
  df$dup.stats<-"singleton";df$dup.stats[which(df$pval < 0.05 & df$delta > 0 )]<-"duplicates"
  if(plot){
...
#' @param d.info data.frame of duplication info generated by dup.snp.info function
...
#' duplicated<-sig.snps(d.info)
#'
#' @export
sig.snps<-function(d.info,plot=TRUE){
  sts<-apply_pb(d.info,1,dupsvd)
  #sts<-dupsvd2(d.info,v95,v05)
