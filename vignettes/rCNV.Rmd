...
This tutorial provides a complete guide to detecting CNVs from SNPs data using rCNV package. This includes importing raw VCF files, filtering VCFs, normalization of read depth, categorization of SNPs into "deviants" and "non-deviants" from which  putative "duplicates" and "singlets" are then filtered. A brief workflow is presented in Fig 1. below.
...
*Please read the function helps for more information on the different methods
...
*p* is the probability of sampling allele *A* in SNP *x* - for unbiased sequencing this is *0.5*. `allele.info()` function calculates this for both *0.5* and biased probability using the ratio between reference and alternative alleles.
...
|   The proportion of heterozygotes in a population according to HWE is 2pq where as homozygotes is q<sup>2</sup>. In deviant SNPs heterozygosity is expected to be higher indicating excess of heterozygosity and thus flagged as deviant/putative-duplicates.
...
We test the validity of detected duplicates in two way: 1. direct detection through a sliding window and 2. variant fixation index

* The sliding window method assess the duplication along chromosomes/scaffolds on a 10,000 bp sliding window. This step assumes that putative duplicates are accompanied by flanking alleles/SNPs withing the sliding window. The putative duplicates that do not follow this assumption will be flagged as low-confident and can be removed if desired.    
** This method is still under beta-testing **

* Variant fixation index ($V_{ST}$) is equivalent to population fixation index ($F_{ST}$) and calculated as $V_{ST} = V_S/V_T$ where $V_T$ is the variance of normalized read depths among all individuals from the two populations and VS is the average of the variance within each population, weighed for population size; and used to identify distinct CNV groups between populations (See Dennis et al., 2017; Weir & Cockerham, 1984)
The function `vst` can be used to calculate the $V_{ST}$ values per population and plot as a network qgraph.
